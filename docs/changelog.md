...
## 2026-09-10（扩展 M1：岗位发现与多主体分类筛选）
- 新增 job_discovery.py：主题关键词搜索 → 岗位分类（方向 + 技术栈）→ 条件筛选 → 打分排序 → 推荐
- 新增 JobPosting / JobSearchTheme 数据模型与 5 个默认主题
- TDD：新增 5 个离线测试，全量 36 passed
- 真实验证：Tavily 主题“AI Agent 应用”→ 推荐 5 个岗位，分类与技术栈打标正确
- 待改进：结果含少量博客页，后续加招聘页特征过滤

## 2026-09-10（扩展 M1.1：来源扩展 + 博客抑制 + 地区筛选）
- 数据来源：企业官网招聘路径 + 招聘平台域名白名单（BOSS直聘/智联/猎聘/51job/拉勾/实习僧/牛客/LinkedIn）
- 博客抑制：默认过滤个人博客与文章帖；按用户要求屏蔽 career.nankai.edu.cn
- 地区筛选：支持 provinces / cities / allow_remote 条件
- TDD：新增 7 个来源与筛选测试，全量 44 passed
- 真实验证：主题“AI Agent 应用”+ 广东省 → 推荐 2 条真实岗位（ncss 校招、牛客岗位），博客/帖子 0 条
- 未实现：按距离筛选（需地理编码，记录为后续项）

## 2026-09-10（扩展 M1.2：中大厂聚焦 + 搜索预算定标）
- 新增公司分层/类型（大厂/中厂/其他；互联网/外企/硬件电子）与分层筛选参数
- 平台白名单扩展（中华英才/大街/赶集/Indeed/Glassdoor），每主题关键词扩到 5 个
