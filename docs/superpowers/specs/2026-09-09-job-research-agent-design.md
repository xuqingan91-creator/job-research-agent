...
上述功能 V1 不实现，但架构不能把它们堵死。每项未来功能都有明确的“接入点”，
V1 只保证接口稳定，不提前实现：

| 未来功能 | 接入点 | V1 需要保持什么 |
|---|---|---|
| Web / Streamlit 界面 | 替换 `cli.py`，复用同一套 graph 调用 | 调研流程与终端无关，graph 返回结构化结果 |
| MCP Server | 把“单次调研”封装为可被其他 Agent 调用的工具 | graph 提供独立入口函数，不依赖交互输入 |
| 多智能体 Supervisor | 在 `nodes.py` 内把角色拆成独立子 Agent | state 可扩展子状态，节点接口单一 |
| 浏览器 / JS 渲染抓取 | `fetch.py` 内部新增 Playwright 实现 | `fetch(url) -> text` 接口不变 |
| 跨会话记忆 / 向量库 | 在持久化层增加会话档案与记忆读写 | 报告落盘与状态序列化独立于内存 |
| PDF / 图片报告 | 报告输出层新增导出器 | 先生成 Markdown 中间产物，再转其他格式 |
| 自动投递简历 | 独立于调研流程的外部动作 | 调研结果只产出报告与建议，不持有投递副作用 |
| 多语言输出 | 输出模板参数化 + prompts 分语言管理 | 报告结构数据化，文案不写死在代码里 |

设计原则：**不提前实现，但每个模块的对外接口保持最小且稳定**——V1 每加一个新模块，
都要能回答“未来某个不做清单功能要从哪里长出来”。
